...
def layer_norm(input_tensor, name=None):
    ...
    # Нормализация ембеддингов переведено из tf 1.0 в tf 2.0
    ...
    with tf.compat.v1.variable_scope("LayerNorm"):
        gamma = tf.compat.v1.get_variable(name="gamma", shape=input_shape[-1:],
                                          initializer=tf.compat.v1.initializers.ones(), trainable=True)
        beta = tf.compat.v1.get_variable(name="beta", shape=input_shape[-1:],
                                         initializer=tf.compat.v1.initializers.zeros(), trainable=True)
...
def attention_layer(from_tensor,
    ...
    # `query_layer` = [B*F, N*H]
    # tf 1.0 -> tf 2.0 dense перенесён в keras.Dense
